...
tabs = ['unfccc-aggregate-up', 'edgar-aggregate-up', 'cait-aggregate-up', 'pik-tp-aggregate-up', 'faostat-aggregate-up',
        'carbon-monitor-aggregate-up', 'gcp-aggregate-up']

inventory_titles = {'unfccc-aggregate-up': 'unfccc', 'edgar-aggregate-up': 'edgar', 'cait-aggregate-up': 'cait',
    'pik-tp-aggregate-up': 'pik-tp', 'faostat-aggregate-up': 'faostat', 'carbon-monitor-aggregate-up': 'carbon-monitor',
...
def generate_sector_comparison_dict():
    ...
    for tab_name in tabs:
        if tab_name in ['pik-tp-aggregate-up', 'carbon-monitor-aggregate-up', 'gcp-aggregate-up']:
            continue
...
def generate_subsector_comparison_dict(tab_name):
    ...
    for comp_sector in comp_sectors:
        ...
        sector_df = df[(df['Comparison Sector'] == comp_sector) & (pd.notna(df['Comparison Subsector']))]
        for index, row in sector_df.iterrows():
            ...
            if tab_name == 'unfccc-aggregate-up':
                ...
                subsector_map[comp_sector][subsector][inventory_titles[tab_name]] = inv_sectors
...
if __name__ == '__main__':
    ...
    for tab in tabs:
        ...
        if tab in ['pik-tp-aggregate-up', 'carbon-monitor-aggregate-up', 'gcp-aggregate-up']:
