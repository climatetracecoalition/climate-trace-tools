...
for item in master_dict_annex1.keys():
    ...
    if not all([inv == 'climate-trace' for inv in invs]) and any([inv == 'climate-trace' for inv in invs]):
...
missing_years = {'unfccc_annex_1': 2021,
...
color_dictionary = {'climate-trace': 'mediumseagreen',
...
# x axis
xaxes = {'gridcolor': '#e6f3ff',
         'linewidth': 1}

# Sets position of each inventory in the plot
legend_rank = {'climate-trace': 1,
                'unfccc_annex_1': 2,
...
total_title = {'climate-trace': 'Total estimate from ClimateTRACE',
...
line_dictionary = {'100-year': 'lines',
                    '20-year': 'lines+markers',
                    'none': 'lines'}
...
gwp_list = {'both': ['100-year', '20-year'],
            '100-year': ['100-year'],
            '20-year': ['20-year'],
            'none': ['none']}
...
def get_legend_title_params(title_dict, comparison_years, sector, key, data_present, nonzero_emissions):
    ...
    if data_present and nonzero_emissions:
        ...
        legend_name = basic_title + f'<span style="font-size: 12px;">  estimate is zero</span>'
...
def is_data_present(item, key):
    ...
    if item.empty is True:
        ...
        if not any([baseline_present.iloc[i] > -1 for i in list(range(len(baseline_present)))]):
            data_present = False
        else:
...
def is_gas_present(key, item):
    gas_presence = {
        'co2': {'data_present': True,
                'nonzero_emissions': True},
        'ch4': {'data_present': True,
                'nonzero_emissions': True},
        'n2o': {'data_present': True,
                'nonzero_emissions': True}}
    for formula in formulas:
        if item.empty:
            ...
            if all([formula_present[column].isnull().all() for column in formula_present.columns]):
                gas_presence[formula]['data_present'] = False
...
def get_ind(title_dict, sector, key, data, column, subsector):
    ...
    ind = int(np.where(
            [title_dict[sector]['legend'][key][data.loc['Data source'][column]]['comps'][tup][0] == subsector
            for tup in
            range(len(title_dict[sector]['legend'][key][data.loc['Data source'][column]]['comps']))])[0])
    return ind

def get_params(title_dict, sector, key, data, column, subsector, trace_type,
               stack_group, co2eq, numerical_data, comparison_years,
               data_present, nonzero_emissions, plot_type, gwp):
    ...
    if trace_type == 'subsector_addition' or trace_type == 'subsector_subtraction' or \
            (trace_type == 'empty' and plot_type != 'gases'):
        ind = get_ind(title_dict, sector, key, data, column, subsector)
        subsector_name = f"{title_dict[sector]['legend'][key][data.loc['Data source'][column]]['comps'][ind][1]}" + \
    ...
    mode_by_co2eq = {'none': 'lines',
                        '100-year': 'lines',
                        '20-year': 'lines',
                        'both': line_dictionary[gwp]}

    gwp_dict = {'both': f' {gwp} GWP',
                 '100-year': '',
                 '20-year': '',
                 'none': ''}

    hover_params = {'namelength': 0,
                    'font_family': 'Foros, medium'}
    traces_dict = {
        'subsector_addition': {'x': x,
                              'y': y,
                              'name': subsector_name,
                              'hovertext': subsector_name + gwp_dict[co2eq],
                              'mode': mode_by_co2eq[co2eq],
                              'line': {'color': color_dictionary[color_type],
                                      'width': 2,
                                      'dash': 'dash'},
        ...
        'total': {'x': x,
                              ...
                              'mode': mode_by_co2eq[co2eq],
                              'line': {'color': color_dictionary[key],
                                       'width': 3},
                              'fillcolor': 'rgba(0,0,0,0)',
                              'fill': 'tonexty',
                              'legendgroup': stack_group,
...
def get_numerical_addition(data_present, nonzero_emissions, data, comparison_years, column, title_dict, sector):
    data_present = True
    nonzero_emissions = True
    startyear = comparison_years[0]
    endyear = comparison_years[-1]
    ...
    if title_dict[sector]['title'].find('Metamodeling') > -1:
        if key in missing_years.keys():
            missingyrs = list(range(startyear, 2016)) + list(range(missing_years['edgar'], endyear))
...
def get_numerical_data(data, comparison_years, column, item, key, title_dict, sector, nonzero_emissions, data_present):
    data_present = True
    nonzero_emissions = True
    startyear = comparison_years[0]
    endyear = comparison_years[-1]
    ...
    numerical_data = data.loc[comparison_years, column].replace(0, np.nan).to_list()
    missingyrs = np.where(
        [item[item['Data source'] == key][comparison_years].isnull().all() for comparison_years in
         item[comparison_years]])[0]
    numerical_data = np.array(numerical_data)
    numerical_data[missingyrs] = np.nan
    numerical_data = numerical_data.tolist()
...
def get_x_and_y(numerical_data, comparison_years):
    ...
    if all(np.isnan(numerical_data)):
        ...
        for row in list(range(len(trace_df))):
            if row == 0:
                if np.logical_not(np.isnan(trace_df.iloc[row, 1])) and np.isnan(trace_df.iloc[row + 1, 1]):
                    points.append(trace_df.iloc[row])
                    continue
            elif row == len(trace_df)-1:
                if np.isnan(trace_df.iloc[row - 1, 1]) and np.logical_not(np.isnan(trace_df.iloc[row, 1])):
                    points.append(trace_df.iloc[row])
                    continue
            elif np.isnan(trace_df.iloc[row-1, 1]) and np.isnan(trace_df.iloc[row+1, 1]):
                if np.logical_not(np.isnan(trace_df.iloc[row, 1])):
                    points.append(trace_df.iloc[row])
        x = np.array(comparison_years[:])[np.where(np.logical_not(np.isnan(numerical_data[:])))[0]]
        y = np.array(numerical_data[:])[np.where(np.logical_not(np.isnan(numerical_data[:])))[0]]
    return x, y, points

def get_missing_subsector_params(title_dict, comparison_years, sector, key, comp):
    startyear = comparison_years[0]
    endyear = comparison_years[-1]
    subsector_name = f"{comp[1]} " \
                     f"{title_dict[sector]['legend'][key][key]['desc']} " \
                     f"{comp[0]}"
    missing_subsector_params = {'x': [startyear],
                      'y': [0],
...
def get_missing_gas_params(key, comparison_years, formula, gas_presence):
    ...
    if not gas_presence[formula]['data_present']:
        ...
        empty_message = ' (estimate is zero)'
    missing_gas_params = {'x': [startyear],
                      'y': [0],
...
def get_points_params(plot_type, color_type, point_symbol, params, co2eq, trace_type):
    try:
        ...
        points_update = {'mode': 'lines+markers',
                         'marker_symbol': point_symbol,
                         'line':{'color': '#d0d6dc'},
                         'marker': {'color': color_dictionary[color_type],
                                    'size': 12},
