...
import numpy as np
import copy as copy
from compare.subtract_out.util.generate_plots import plot
...
def create_plots(allinv, countries, sector, gas, co2eq, plot_type, ratio_data, output_folder, comparison_dicts, title_dicts, create_folders, plot_live):
    ...
    for country in countries:
        ...
        if not (all(plotting_dict[d].empty for d in plotting_dict.keys()) or plotting_dict['climate-trace'].empty):
            try:
                plot(sector, country, gas, co2eq, plot_type, title_dict, output_folder, plotting_dict, create_folders, plot_live)
