...
import numpy as np
import os
from compare.subtract_out.util.plotting_utils import (
    get_layout,
    fonts,
    get_yaxes,
    xaxes,
...
def plot(
    ...
    if (
        ...
        & ("climate-trace" in non_zero_sum.keys())
        & (sum(data_present_sum.values()) > 1)
    ):
        pass
    else:
        print(
            ...
            if plot_type == "subsectors":
                if not data_present or not nonzero_emissions:
                    for comp in title_dict[sector]["legend"][key][key]["comps"]:
                        params = get_missing_subsector_params(
                            title_dict, comparison_years, sector, key, comp
                        )
                ...
                for column in data.columns:
                    if gas == "all":
                        if not (
                            data.loc["Gas", column] == "co2e"
                            and data.loc["carbon_eq", column] == gwp
                        ):
                            continue
                    else:
                        if not (
                            data.loc["Gas", column] == gas
                            and data.loc["carbon_eq", column] == gwp
                        ):
                            continue
                    ...
                    if (
                        ...
                        numerical_data, nonzero_emissions, data_present = (
                            get_numerical_addition(
                                data_present,
                                nonzero_emissions,
                                data,
                    ...
                    if (
                        gas == "all" and data.loc["Data source"][column] == "gapfilled"
                    ) or (
                        gas != "all"
                        and data.loc["Data source"][column] == "gapfilled"
