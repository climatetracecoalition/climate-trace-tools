...
def calculate_gwp(datasourcedf):
    ...
    """
    cols = ['iso3_country', 'reporting_entity', 'start_time', 'end_time', 'original_inventory_sector']
    ...
    df_pivot = pd.pivot_table(data=datasourcedf, values='emissions_quantity', index=cols, columns='gas',
    ...
    df_pivot_melt = df_pivot.melt(id_vars=cols,
                                  var_name="gas",
...
def parse_and_format_query_data(df, years_to_columns=True, rename_columns=True, times_to_years=True):
    ...
    df = df.groupby(['original_inventory_sector','iso3_country', 'reporting_entity', 'gas', 'emissions_quantity_units', 'year'], as_index=False).sum()

    if rename_columns:
        df = df.rename(columns = DB_SOURCE_TO_COL_NAME)

    if years_to_columns:
        transformed_df = df.pivot(index=["Sector", "ID", "Data source", "Gas", "Unit"], columns='year', values='emissions_quantity').reset_index()
...
def load_data():
    ...
    all_data = all_data[['original_inventory_sector', 'iso3_country', 'reporting_entity', 'gas', 'emissions_quantity', 'emissions_quantity_units', 'start_time', 'end_time']]
