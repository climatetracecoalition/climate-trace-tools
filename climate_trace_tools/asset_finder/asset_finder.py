...
def find_assets(location, buffer_zone=5, year=2024, gas='co2e_100yr', sectors='all', country=None, asset_cols=None):
    """
    ...
    FINAL_COLS = \
        ['location', 'origin', 'source_id', 'source_name', 'source_type', 'iso3_country', 
         'source_lat_lon', 'source_km_from_origin', 'buffer', 'subsector', 'year', 'num_months', 'gas', 
         'activity', 'activity_units', 'emissions_quantity', 'emissions_factor', 'emissions_factor_units',
         'gadm_0', 'gadm_0_name', 'gadm_0_activity', 'gadm_0_emissions_quantity', 'gadm_0_emissions_factor',
         'gadm_1', 'gadm_1_name', 'gadm_1_activity', 'gadm_1_emissions_quantity', 'gadm_1_emissions_factor', 
         'gadm_2', 'gadm_2_name', 'gadm_2_activity', 'gadm_2_emissions_quantity', 'gadm_2_emissions_factor']
    ...
    if location.strip().upper().startswith(('POLYGON', 'MULTIPOLYGON')):
        ...
        try:
            ...
            raise ValueError(f"Invalid location string format: '{location}'. Must be 'lat, lon' or POLYGON. Error: {e}")
    print('\n***')
    print(location)
    ...
    if location_type == 'coordinate':
        ...
        buffer_zone_geom = f"ST_Buffer(ST_GEOGPOINT({lon}, {lat}), {buffer_zone} * 1000)"
        
    elif location_type == 'area':
        location_zone = f"ST_GEOGFROMTEXT('{location}')"
        buffer_zone_geom = f"ST_Buffer(ST_GEOGFROMTEXT('{location}'), {buffer_zone} * 1000)"
    ...
    if country is not None:
        ...
        assert not countries.empty, 'Location exists outside of the boundaries of Climate Trace data'
    ...
    for country in countries:
        ...
        if new_assets.empty:
            ...
            for gadm_col_prefix in ['gadm_0', 'gadm_1', 'gadm_2']:
                print('Querying %s for emissions within the same gadm + sector' % gadm_col_prefix)
                ...
                base_expr = "ARRAY_TO_STRING(ARRAY_SLICE(SPLIT(ge.gadm_id, '.'), 0, " + str(gadm_num) + "), '.')"
                ...
                if not gadm_df.empty:
                    gadm_df = gadm_df[[gadm_col_prefix, 'gadm_name', 'gadm_activity', 'gadm_emissions_quantity', 'subsector']]
                    gadm_df = gadm_df.rename(columns={'gadm_name': gadm_name,
                                                      'gadm_activity': gadm_activity,
                                                      'gadm_emissions_quantity': gadm_emissions})

                    new_assets = new_assets.merge(gadm_df, how='left', on=[gadm_col_prefix, 'subsector'])
                else:
                    print(f"No GADM data found for {asset_sectors} in {gadm_col_prefix}, skipping merge.")
    ...
    if not asset_df.empty:
        ...
        asset_df['source_km_from_origin'] = asset_df.apply(lambda row: geodesic((row['origin'].y, row['origin'].x), (row['source_lat_lon'].y, row['source_lat_lon'].x)).meters, axis=1) / 1000
        asset_df['origin'] = asset_df['origin'].apply(lambda p: f"{p.y}, {p.x}")
        asset_df['source_lat_lon'] = asset_df['source_lat_lon'].apply(lambda p: f"{p.y}, {p.x}")
        asset_df['emissions_factor'] = asset_df['emissions_quantity'] / asset_df['activity']
        asset_df['gadm_0_emissions_factor'] = asset_df['gadm_0_emissions_quantity'] / asset_df['gadm_0_activity']
        asset_df['gadm_1_emissions_factor'] = asset_df['gadm_1_emissions_quantity'] / asset_df['gadm_1_activity']
        asset_df['gadm_2_emissions_factor'] = asset_df['gadm_2_emissions_quantity'] / asset_df['gadm_2_activity']
