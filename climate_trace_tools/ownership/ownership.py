...
def _load_csv_from_zip(zf, pattern_include, pattern_exclude=None):
    ...
    matches = [
        ...
        if pattern_include in n and n.endswith('.csv')
...
def load_ownership_data(ownership_zip):
    ...
    with zipfile.ZipFile(ownership_zip, 'r') as zf:
        ...
        all_entity_connections_df = _load_csv_from_zip(zf, 'entity_relationships', 'asset')

    asset_to_owner_df = asset_to_owner_df[asset_to_owner_df['source_id'].notna()]
    asset_to_owner_df['source_id'] = asset_to_owner_df['source_id'].astype(int).astype(str)
...
def build_ownership_graph(asset_to_owner_df, all_entities_df, all_entity_connections_df):
    ...
    for _, row in all_entity_connections_df.iterrows():
        ...
        if pd.notna(owner_id) and pd.notna(subject_id):
            ...
            G.add_edge(owner_id, subject_id, relation_type='owns_entity',
...
def get_assets_owned_by_entity(entity_name, ownership_zip, gas='co2e_100yr', year=2024):
    ...
    df = df.merge(
        asset_to_owner_df[['source_id', 'source_name', 'source_sector', 'source_subsector']],
    ...
    owners_emissions['source_id'] = owners_emissions['source_id'].astype(int).astype(str)
